...
const List = () => {
  ...
  useEffect(() => {
    dispatch(clearVisitID()); // پاک کردن visitI
    console.log(id)

    if (isAuthenticated && token) {
      dispatch(fetchItems(token, id));
    }
  ...
  const filteredItems = items
    .filter(item =>
      item.unvan.toLowerCase().includes(search.toLowerCase()) ||
      item.musterino.toLowerCase().includes(search.toLowerCase())
    )
    .filter(item => flowFilter === '' || item.flow === flowFilter || (flowFilter === 'unvisited' && !item.flow));
      ...
      <div className="max-w-4xl mx-auto px-4 py-8" style={{ direction: 'rtl' }}>
        ...
        <div className="mb-4 flex flex-col md:flex-row md:justify-between gap-4">
          ...
          >
            ...
            <option value="unvisited">ویزیت نشده</option>
          </select>
        </div>
        <div className="mb-4">
          <span>تعداد مشتریان ویزیت نشده: {unvisitedCount}</span>
        </div>
          ...
          {sortedItems.map((item, index) => (
            ...
            >
              ...
              <p className="text-gray-600">
                {item.flow ? `وضعیت : ${item.flow}` : 'بدون فعالیت'}
              </p> 
            </div>
          ))}
        </div>
...
import { useSelector, useDispatch } from 'react-redux';
import { useEffect, useState } from 'react';
import { fetchAllCustomers, fetchItems, selectItem, selectUnvisitedCount } from '../../../lib/store/itemsSlice';
import { clearVisitID } from '../../../lib/store/itemSlice';
import { useRouter, useParams } from 'next/navigation';
import Layout from '@/components/Layout';
import AuthWrapper from '@/components/AuthWrapper';
...
const List = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // Add logging to check `id`, `isAuthenticated`, and `token`
     // console.log('Fetching items with ID:', id);
    //  console.log('Is authenticated:', isAuthenticated);
    //  console.log('Token:', token);

      if (isAuthenticated && token) {
       // console.log('Fetching items with ID:', id);
        await dispatch(fetchAllCustomers(token, id));
      }
    };

    fetchData();
  }, [dispatch, isAuthenticated, token, id]); // Update dependency array
  ...
  // Handle cases where `items` might be empty or not yet populated
  const filteredItems = items
    .filter(item =>
      item.unvan?.toLowerCase().includes(search.toLowerCase()) ||
      item.musterino?.toLowerCase().includes(search.toLowerCase())
  ...
  const handleClick = (item) => {
    if(role == 'Specialist'){
      dispatch(selectItem(item));
      router.push(`/details/${item.musterino}`);
    } 
     if(role == 'Visitor'){
    dispatch(selectItem(item));
    router.push(`/detailVisitor/${item.musterino}`);
  ...
  const approvedPercentage = totalCustomers > 0 ? ((approvedCount / totalCustomers) * 100).toFixed(2) : 0;
        ...
        <div className="max-w-4xl mx-auto px-4 py-8" style={{ direction: 'rtl' }}>
          ...
          <div className="mb-4 flex flex-col md:flex-row md:justify-between gap-4">
            ...
            >
              ...
              {/* <option value="بازدید نشده">باز‌دید نشده</option> */}
            </select>
          </div>
       {/*    <div className="mb-4">
            <span>تعداد مشتریان ویزیت نشده: {unvisitedCount}</span>
            ...
            {sortedItems.length > 0 ? (
              sortedItems.map((item, index) => (
                <div
                  ...
                  style={{ textAlign: 'right' }}
                >
                  <h2 className="text-2xl font-semibold text-gray-800 mb-4">{item.unvan || 'بدون نام'}</h2>
                  <p className="text-gray-600 mb-2">{item.musterino || 'بدون شماره'}</p>
              ...
              <p className="text-center text-gray-600">هیچ مشتری یافت نشد.</p>
            )}
          </div>
        </div>
      </AuthWrapper>
