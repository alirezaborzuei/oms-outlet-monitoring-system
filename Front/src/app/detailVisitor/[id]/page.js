...
import { fetchItemId, checkVisitStatus, clearVisitID } from '@/lib/store/itemSlice';
...
const CustomerDetail = ({ params }) => {
  ...
  useEffect(() => {
    ...
    //  dispatch(checkVisitStatus({ token, customerCode: musterino }));
    }
  ...
  const handleBack = () => {
    ...
    router.back(); // Redirect to the desired page
  };
        ...
        <div className="max-w-2xl mx-auto p-6 bg-white shadow-lg rounded-lg mt-10" style={{ direction: 'rtl' }}>
          ...
          <div className="mb-4 mt-4"> 
            <div className="mb-4 mt-4 flex justify-between space-x-4 rtl:space-x-reverse"> 
              {role === "Visitor" && (
                <button
                  onClick={() => router.push(`/detailVisitor/${musterino}/new-visit`)} // redirect to other page becuase url parameter save and pass to detail.
                  className="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-700"
                >
                  {visitID ? 'ادامه بازدید' : 'افزودن بازدید جدید'}
                </button>
              )}
              <button
                onClick={handleBack}
                ...
                <li key={visit.visitID} className="p-4 bg-white shadow-md rounded-lg">
                  <div className="flex justify-between items-center">
                    <div>
                      ...
                      <span className="block text-sm text-gray-600">{`وضعیت: ${flowDisplayMap[visit.flow] || 'نامشخص'}`}</span>
                    </div>
                    <a 
                      href={`/photo/${visit.visitID}`}
